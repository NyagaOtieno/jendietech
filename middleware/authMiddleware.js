...
import { isValidKenyanPhone, normalizeKenyanPhone } from "../utils/phone.util.js";
...
export const register = async (req, res) => {
  try {
    let { name, email, phone, password, role, schoolId } = req.body;

    if (!name || !password || (!email && !phone) || !role || !schoolId) {
      return res.status(400).json({
        error: "Name, password, role, schoolId, and at least email or phone are required",
      });
    }
